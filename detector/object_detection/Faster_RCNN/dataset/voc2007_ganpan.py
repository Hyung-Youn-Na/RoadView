...
from detector.object_detection.Faster_RCNN.dataset.base import Base
from detector.object_detection.Faster_RCNN.voc_eval import voc_eval


class VOC2007Ganpan(Base):
    ...
    def __init__(self, path_to_data_dir: str, mode: Base.Mode, image_min_side: float, image_max_side: float):
        ...
        path_to_voc2007_dir = os.path.join(self._path_to_data_dir)
        ...
        print(path_to_imagesets_main_dir)
        print(path_to_annotations_dir)
        print(self._path_to_jpeg_images_dir)

        if self._mode == VOC2007Ganpan.Mode.TRAIN:
            path_to_image_ids_txt = os.path.join(path_to_imagesets_main_dir, 'trainval.txt')
        elif self._mode == VOC2007Ganpan.Mode.EVAL:
        ...
        with open(path_to_image_ids_txt, 'r') as f:
            lines = f.readlines()
            image_ids = [os.path.basename(line.split()[0]).split('.')[0] for line in lines]
        ...
        for image_id in image_ids:
            ...
            annotation = VOC2007Ganpan.Annotation(
                filename=root.find('filename').text,
                objects=[VOC2007Ganpan.Annotation.Object(
            ...
            annotation.objects = [obj for obj in annotation.objects if obj.name in ['text'] and not obj.difficult]
    ...
    def __getitem__(self, index: int) -> Tuple[str, Tensor, Tensor, Tensor, Tensor]:
        try:
            if index >= len(self._image_ids):
                index = len(self._image_ids) - 1
        ...
        labels = [VOC2007Ganpan.CATEGORY_TO_LABEL_DICT[obj.name] for obj in annotation.objects]
        ...
        if self._mode == VOC2007Ganpan.Mode.TRAIN and random.random() > 0.5:
            image = ImageOps.mirror(image)
            bboxes[:, [0, 2]] = image.width - bboxes[:, [2, 0]]  # index 0 and 2 represent `left` and `right` respectively

        image, scale = VOC2007Ganpan.preprocess(image, self._image_min_side, self._image_max_side)
    ...
    def evaluate(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]) -> Tuple[float, str]:
        ...
        class_to_ap_dict = {}
        for c in range(1, VOC2007Ganpan.num_classes()):
            category = VOC2007Ganpan.LABEL_TO_CATEGORY_DICT[c]
            try:
                path_to_cache_dir = os.path.join('caches', 'voc2007-ganpan')
        ...
        detail = ''
        for c in range(1, VOC2007Ganpan.num_classes()):
            detail += '{:d}: {:s} AP = {:.4f}\n'.format(c, VOC2007Ganpan.LABEL_TO_CATEGORY_DICT[c], class_to_ap_dict[c])
    ...
    def _write_results(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]):
        ...
        for c in range(1, VOC2007Ganpan.num_classes()):
            class_to_txt_files_dict[c] = open(os.path.join(path_to_results_dir, 'comp3_det_test_{:s}.txt'.format(VOC2007Ganpan.LABEL_TO_CATEGORY_DICT[c])), 'w')
