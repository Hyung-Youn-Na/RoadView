...
class Base(torch.utils.data.dataset.Dataset):
    ...
    OPTIONS = ['voc2007-ganpan']

    @staticmethod
    def from_name(name: str) -> Type['Base']:

        if name == 'voc2007-ganpan':
            from detector.object_detection.Faster_RCNN.dataset.voc2007_ganpan import VOC2007Ganpan
            return VOC2007Ganpan
        else:
            raise ValueError
